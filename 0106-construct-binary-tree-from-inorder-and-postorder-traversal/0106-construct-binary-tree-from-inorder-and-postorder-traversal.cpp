...
public:
    TreeNode* solve(vector<int>& inorder, int instart, int inend, vector<int>&postorder, int poststart, int postend, map<int,int>&mp){
        ...
        root->left = solve(inorder, instart, inroot-1, postorder, poststart, poststart+len-1,mp);
        root->right = solve(inorder, inroot+1, inend, postorder, poststart+len, postend-1,mp);
        return root;
    }
